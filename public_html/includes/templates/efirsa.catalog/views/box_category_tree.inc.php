  ...
  if (!function_exists('custom_draw_category_tree')) {
    function custom_draw_category_tree($categories, $indent=0) {
      foreach ($categories as $category) {
        echo '  <li  type = "circle" class="category-'. $category['id'] . (!empty($category['active']) ? ' active' : '') .'">' . PHP_EOL
           . '    <a href="'. htmlspecialchars($category['link']) .'"><i class="'. (empty($category['active']) ? 'plus' : 'minus') .'-square" style="font-size: 0.75em;"></i> '. $category['name'] .'</a>' . PHP_EOL;
